...
retry:			try { _form.Invoke(new MethodInvoker(() => { var h = _form.Handle; })); } catch { Thread.Sleep(10); goto retry; }
		...
		public async Task HandleChat_WhenModelLoaded_AttemptsResponse(){
			using(var client = new HttpClient()){
				var response = await client.PostAsync($"http://localhost:{TestPort}/v1/responses", JsonContent(Json.Object(Json.P("messages", Json.Array(Json.Object(Json.P("role", "user"), Json.P("content", "Hello")))))));
		...
		public async Task HandleReset_WithSessionId_RemovesSession(){
			string sessionId;
			using(var client = new HttpClient()){
				var response = await client.PostAsync($"http://localhost:{TestPort}/v1/responses", JsonContent(Json.Object(Json.P("messages", Json.Array(Json.Object(Json.P("role", "user"), Json.P("content", "Hi")))))));
			...
			using(var client = new HttpClient()){
				var response = await client.PostAsync($"http://localhost:{TestPort}/v1/reset", JsonContent(Json.Object(Json.P("session_id", sessionId))));
		...
		public async Task HandleReset_GlobalScope_ClearsAllSessions(){
			...
			using(var client = new HttpClient()){
				var response = await client.PostAsync($"http://localhost:{TestPort}/v1/reset", JsonContent(Json.Object(Json.P("scope", "global"))));
