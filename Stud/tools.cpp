...
static std::string GetArgValue(const char* args, const char* key){
	if(!args || !key) return "";
...
void RegisterTools(const bool dateTime, const bool googleSearch, const bool webpageFetch, const bool fileList, const bool fileCreate, const bool fileRead, const bool fileWrite){
	...
	if(fileRead){ AddTool("file_read_lines", "Display range of lines from a text file with line numbers", "{\"type\":\"object\",\"properties\":{\"path\":{\"type\":\"string\"},\"start\":{\"type\":\"integer\"},\"end\":{\"type\":\"integer\"}},\"required\":[\"path\"]}", ReadFileTool); }
	if(fileWrite){
