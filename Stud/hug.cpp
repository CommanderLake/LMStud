...
static bool ResolveSystemProxyForUrl(const char* url, std::string& outProxy){
	...
	if(ie.fAutoDetect || ie.lpszAutoConfigUrl){
		...
		ok = WinHttpGetProxyForUrl(nullptr, std::wstring(url, url + strlen(url)).c_str(), &ao, &pi);
	}
	if(!ok && ie.lpszProxy){
...
char* PerformHttpGet(const char* url){
	...
	try{
		...
		std::string proxy;
		if(ResolveSystemProxyForUrl(url, proxy)){ curl_easy_setopt(curl, CURLOPT_PROXY, proxy.c_str()); }
