...
namespace LMStud {
	...
	internal sealed class GdiMarkdownRenderer : IDisposable {
		...
		private float AddTable(string[] lines, ref int i, float y, int width) {
			...
			i--;
			var cols = 0;
			foreach(var row in rows) if(row.Length > cols) cols = row.Length;
			if(cols == 0) return y;
			...
			for(var c = 0; c < cols; c++) {
				...
				_ops.Add(DrawOp.Rect(rect.X, rect.Y, rect.Width, rect.Height, r == 0 ? Color.FromArgb(240, 240, 240) : Color.FromArgb(248, 248, 248), true));
		...
		private float AddCodeBlock(string code, float y, int width) {
			...
			_ops.Add(DrawOp.Rect(0, y, width, blockHeight, Color.FromArgb(245, 245, 245), true));
			...
			foreach(var run in runs){
				...
				foreach(var drawText in EnumerateDrawTokens(run.Text)) {
					...
					if(run.Kind == InlineKind.Code) {
						_ops.Add(DrawOp.Rect(rect.X - 1, rect.Y + 1, rect.Width + 2, rect.Height - 2, Color.FromArgb(245, 245, 245), true));
						_ops.Add(DrawOp.StrokeRect(new RectangleF(rect.X - 1, rect.Y + 1, rect.Width + 2, rect.Height - 2), Color.FromArgb(225, 225, 225)));
					}
					_ops.Add(DrawOp.Text(drawText, rect, font, color, run.Strike));
		...
		private Font ResolveRunFont(Font paraFont, InlineRun run) {
			...
			return GetFont(style, paraFont.Size, "Segoe UI Symbol");
		}
		...
		private static int FindNextSpecial(string text, int start) {
			for(var i = start; i < text.Length; i++) {
				...
				if((ch == 'h' || ch == 'H') && i + 7 < text.Length)
					if(text.IndexOf("http://", i, StringComparison.OrdinalIgnoreCase) == i || text.IndexOf("https://", i, StringComparison.OrdinalIgnoreCase) == i) return i;
			}
			return text.Length;
		}
		...
		internal string HitTestLink(Point point){
			foreach(var t in _linkRegions)
				if(t.Rect.Contains(point)) return t.Url;
			return null;
		}
		...
		private sealed class LinkRegion {
			...
			internal LinkRegion(RectangleF rect, string url) { Rect = rect; Url = url; }
		}
		private enum InlineKind { Text, Code, Break }
		...
		private sealed class DrawOp {
			...
			internal static DrawOp Text(string text, RectangleF rect, Font font, Color color, bool strike = false) {
				return new DrawOp(g => {
					TextRenderer.DrawText(g, text, font, Rectangle.Round(rect), color, TextFormatFlags.NoPadding | TextFormatFlags.NoClipping);
					if(strike)
			...
			internal static DrawOp StrokeRect(RectangleF rect, Color color) {
				return Rect(rect.X, rect.Y, rect.Width, rect.Height, color, false);
			}
